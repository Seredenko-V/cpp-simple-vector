...
namespace tests {
    ...
    void TestTemporaryObjConstructor() {
        ...
        cerr << "Test with temporary object, copy elision"s << endl;
    ...
    void TestTemporaryObjOperator() {
        const size_t size = 1000000;
        cerr << "Test with temporary object, operator="s << endl;
    ...
    void TestNamedMoveConstructor() {
        const size_t size = 1000000;
        cerr << "Test with named object, move constructor"s << endl;
    ...
    void TestNamedMoveOperator() {
        const size_t size = 1000000;
        cerr << "Test with named object, operator="s << endl;
        SimpleVector<int> vector_to_move(GenerateVector(size));
        assert(vector_to_move.GetSize() == size);
    ...
    void TestNoncopiableMoveConstructor() {
        const size_t size = 5;
        cerr << "Test noncopiable object, move constructor"s << endl;
    ...
    void TestNoncopiablePushBack() {
        const size_t size = 5;
        cerr << "Test noncopiable push back"s << endl;
        SimpleVector<X> v;
        for (size_t i = 0; i < size; ++i) {
            v.PushBack(X(i));
        }
    ...
    void TestNoncopiableInsert() {
        const size_t size = 5;
        cerr << "Test noncopiable insert"s << endl;
        SimpleVector<X> v;
        for (size_t i = 0; i < size; ++i) {
            v.PushBack(X(i));
        }
    ...
    void TestNoncopiableErase() {
        const size_t size = 3;
        cerr << "Test noncopiable erase"s << endl;
        SimpleVector<X> v;
        for (size_t i = 0; i < size; ++i) {
            v.PushBack(X(i));
        }
